...
//    }
    ...
    public Zone loadDataFromGeneratedData (SuperDuperMarketDescriptor superDuperMarketDescriptor) {
        ...
        String zone = GENERATED_DATA_MAPPER.generatedZoneToZone(superDuperMarketDescriptor.getSDMZone());
        FILE_MANAGER_VALIDATOR.validateItemsAndStores(items, stores);

        return GENERATED_DATA_MAPPER.toZone(items, stores, zone);
    }

    public void saveOrdersHistoryToFile (Zone zone, String path) {
        if (zone == null) {
            throw new FileNotLoadedException();
        }

        try {
            SystemOrdersHistory systemOrdersHistory = new SystemOrdersHistory(zone.getSystemOrders());
